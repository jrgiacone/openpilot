...
from openpilot.system.hardware.power_monitoring import PowerMonitoring, VBATT_LOW_POWER_ENTRY, VBATT_LOW_POWER_EXIT
...
SCREEN_OFF_IDLE_S = 300  # FastSleep: blank the panel this long after parking or the last touch
SCREEN_WAKE_DEFAULT_PCT = 50  # brightness to restore if the pre-blank value is unknown
...
def hardware_thread(end_event, hw_queue) -> None:
  ...
  sm = messaging.SubMaster(["peripheralState", "gpsLocationExternal", "selfdriveState", "pandaStates", "touch"], poll="pandaStates")
  ...
  screen_off = False
  saved_brightness_pct = SCREEN_WAKE_DEFAULT_PCT
  last_touch_mono = 0.0
  ...
  while not end_event.is_set():
    ...
    if msg.deviceState.memoryUsagePercent > 85:
      ...
      perf.emit(
        "hardware_low_memory",
        severity="error" if msg.deviceState.memoryUsagePercent > 90 else "warning",
        frame_id=sm.frame,
        samples=[PerfSample(
          frame_id=sm.frame,
    ...
    now = time.monotonic()
    tap_wake_ok = power_monitor.car_voltage_mV >= (VBATT_LOW_POWER_ENTRY * 1e3)
    if sm.updated['touch'] and tap_wake_ok:
      last_touch_mono = now

    want_screen_off = low_power
    if fast_sleep and not tesla_no_sleep and not onroad_conditions["ignition"] and started_ts is None:
      idle_ref = max(off_ts or now, last_touch_mono)
      if (now - idle_ref) > SCREEN_OFF_IDLE_S:
        want_screen_off = True
    if onroad_conditions["ignition"] or started_ts is not None:
      want_screen_off = False

    if want_screen_off:
      if not screen_off:
        saved_brightness_pct = HARDWARE.get_screen_brightness() or saved_brightness_pct
      HARDWARE.set_screen_brightness(0)  # re-assert each loop so the UI can't relight it
    elif screen_off:
      HARDWARE.set_screen_brightness(saved_brightness_pct or SCREEN_WAKE_DEFAULT_PCT)
    screen_off = want_screen_off
