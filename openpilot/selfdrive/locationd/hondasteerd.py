...
def fill_msg(model: HondaSteeringModel, valid: bool, lagd_delay: float = 0.0,
  ...
  p.effectiveLag = float(model.effective_lag)
  p.delayLearned = bool(model.delay_learned)
  p.delayRailed = bool(model.delay_railed)
  ...
  p.gainResets = int(model.gain_resets)
  p.termResets = int(model.term_resets)
  p.steerRatioResets = int(model.steer_ratio_resets)
