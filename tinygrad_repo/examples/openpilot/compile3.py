...
_out = _sys.argv[-1]
if _out.endswith('_tinygrad.pkl'):
    _st = _os.path.join('/tmp/iqp_model_stash', _os.path.basename(_out))
    if _os.path.exists(_st) and _os.path.getsize(_st) > 0:
        _os.makedirs(_os.path.dirname(_out) or '.', exist_ok=True)
        _sh.copyfile(_st, _out)
        print('[prebuilt] restored device-compiled model:', _out); _sys.exit(0)
