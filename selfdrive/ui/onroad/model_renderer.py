...
class ModelRenderer(Widget, IQModelRenderer):
  ...
  def _draw_lead_indicator(self):
    # Soft teal glowing orb, centered on the lead. Stays teal; overall intensity (opacity) rises as
    # the lead gets closer / we close on it (fill_alpha), and the orb grows with proximity (radius).
    src = rl.Rectangle(0, 0, self._lead_orb.width, self._lead_orb.height)
    for lead in self._lead_vehicles:
      ...
      dest = rl.Rectangle(cx, cy, r * 2.0, r * 2.0)
      rl.draw_texture_pro(self._lead_orb, src, dest, rl.Vector2(r, r), 0.0, rl.Color(255, 255, 255, alpha))
