...
from openpilot.selfdrive.ui.mici.widgets.stock_dialog import BigDialog
...
class InstallModePage(NavScroller):
  ...
  def __init__(self, on_select: Callable[[str], None]):
    ...
    for mode, label in self.MODES:
      btn = BigButton(label, "", check_icon if mode == current_mode else None, scroll=True)
...
class InstallModeButton(BigButton):
  ...
  def _update_state(self):
    super()._update_state()
    self.set_value(self.MODE_LABELS[self._current_mode()])
...
class DisableUpdatesButton(BigButton):
  ...
  def _sync_from_params(self):
    disabled = ui_state.params.get_bool("DisableUpdates")
    self.set_text("enable\nupdates" if disabled else "disable\nupdates")
...
class SoftwareLayoutMici(NavScroller):
  def __init__(self):
    ...
    uninstall_openpilot_btn = EngagedConfirmationButton("uninstall IQ.Pilot", "uninstall",
