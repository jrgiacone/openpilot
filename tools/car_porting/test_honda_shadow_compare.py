...
# tools/car_porting has no __init__.py, so this collects like its sibling modules do -
# unqualified, not via the openpilot.* namespace (openpilot/tools mirrors only a subset of
# the top-level tools/ tree, car_porting is not part of it)
...
def test_a_model_scores_itself_near_zero():
...
def test_a_wrong_model_scores_worse_than_the_truth():
...
def test_bucket_rms_matches_overall_when_one_speed():
...
def test_a_saturated_sample_is_not_scored():
...
def test_ground_truth_prefers_yaw_over_kinematic():
...
def test_ground_truth_removes_road_roll():
